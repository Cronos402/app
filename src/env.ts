...
const clientEnvSchema = z.object({
  ...
  NEXT_PUBLIC_API2_URL: z.string().url(),
  NEXT_PUBLIC_FACILITATOR_URL: z.string().url().optional(),
});

// Build the raw env object using individual references so Next can inline them
// IMPORTANT: Local development fallbacks - NO production URLs!
const rawClientEnv = {
  NODE_ENV: process.env.NODE_ENV,
  NEXT_PUBLIC_AUTH_URL: process.env.NEXT_PUBLIC_AUTH_URL ?? 'http://localhost:3005',
  NEXT_PUBLIC_MCP2_URL: process.env.NEXT_PUBLIC_MCP2_URL ?? 'http://localhost:3006',
  NEXT_PUBLIC_MCP_PROXY_URL: process.env.NEXT_PUBLIC_MCP_PROXY_URL ?? 'http://localhost:3005',
  NEXT_PUBLIC_MCP_DATA_URL: process.env.NEXT_PUBLIC_MCP_DATA_URL ?? 'http://localhost:3006',
  NEXT_PUBLIC_API2_URL: process.env.NEXT_PUBLIC_API2_URL ?? 'http://localhost:3001',
  NEXT_PUBLIC_FACILITATOR_URL: process.env.NEXT_PUBLIC_FACILITATOR_URL ?? 'http://localhost:3004',
};
