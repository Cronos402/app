...
type Templates = {
  json: (ctx: { baseUrl: string; apiKey?: string; serverName: string }, authMode?: AuthMode, platform?: 'mac' | 'win' | 'wsl') => string
...
const defaultTemplates: Templates = {
  json: ({ baseUrl, apiKey, serverName }, authMode = 'api_key', platform = 'mac') => {
    const getCommand = () => {
...
const generateRaycastDeepLink = (server: ServerInfo, apiKey?: string) => {
  const config = {
    ...
    type: "stdio",
    command: "npx",
    args: ["cronos402", "connect", "--urls", server.baseUrl]
  }
                ...
                <div className="space-y-3">
                  <div className="text-xs text-muted-foreground">
                    {state.codeLanguage === 'ts'
                      ? 'npm install @modelcontextprotocol/sdk cronos402 x402'
                      : 'pip install mcp'
                    }
                  </div>
