...
import { CRONOS_NETWORK, type CronosNetworkId } from "@/lib/constants/cronos"
// Cronos-only networks - uses centralized constants
const CronosNetworks = [CRONOS_NETWORK.CRONOS, CRONOS_NETWORK.TESTNET] as const
type CronosNetwork = CronosNetworkId
...
export function MonetizeWizard({ open, onOpenChange, serverUrl, tools, initialAuthHeaders, initialRequireAuth, onCreate }: MonetizeWizardProps) {
  ...
  const isTestnetNetworkName = (n: string) => /testnet/i.test(n)
  // Cronos-only: no SVM networks
  const evmNetworks = CronosNetworks
  const svmNetworks: readonly string[] = [] // No SVM for Cronos
  ...
  const visibleSvmNetworks: string[] = [] // No SVM for Cronos
  ...
  const needsEvm = useMemo(() => selectedNetworks.some(n => (evmNetworks as readonly string[]).includes(n)), [selectedNetworks, evmNetworks])
  const needsSvm = useMemo(() => false, []) // No SVM for Cronos
  ...
  // Filter wallets by architecture - only EVM (Cronos) supported
  const evmWallets = useMemo(() =>
    userWallets.filter(wallet => getBlockchainArchitecture(wallet.blockchain) === 'evm'),
    [userWallets]
  )
  // SVM (Solana) not supported - always empty
  const svmWallets: typeof userWallets = []
  ...
  // Set default selections to primary wallet (Cronos EVM only)
  useEffect(() => {
    if (primaryWallet && !evmRecipientAddress && !evmManuallyCleared && !evmUserEdited) {
      ...
      // Note: SVM (Solana) is not supported in Cronos402
    }
  }, [primaryWallet, evmRecipientAddress, needsEvm, evmManuallyCleared, evmUserEdited])
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {/* SVM section hidden - Cronos402 only supports EVM (Cronos) networks */}
            </div>
          </div>
        )}
                  ...
                  <Popover open={evmWalletSelectorOpen} onOpenChange={setEvmWalletSelectorOpen}>
                    {evmWallets.length === 0 ? (
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>Connect to Cronos402 to select a wallet from here</p>
                        </TooltipContent>
                      </Tooltip>
                    ) : (
                      <PopoverTrigger asChild>
