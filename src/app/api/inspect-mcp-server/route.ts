...
async function inspect(url: string, include?: IncludeParam, authHeaders?: Record<string, string>) {
  ...
  const [toolsResult, prompts] = await Promise.all([
    doTools ? getToolsAndServerInfo(url, authHeaders).catch((err) => { console.warn('tools fetch failed', err); return { tools: [], serverInfo: null } }) : Promise.resolve(undefined),
  ...
  const serverInfo = toolsResult?.serverInfo

  return { url, tools, prompts, serverInfo }
}

async function getToolsAndServerInfo(url: string, authHeaders?: Record<string, string>) {
  try {
    ...
    // Use the low-level client to get server info from initialize
    const client = new Client({ name: "cronos402-inspect", version: "1.0.0" })
    await client.connect(transport)

    // Get server info from the client (populated during initialize)
    ...
    return { tools: [], serverInfo: null }
  }
}

async function getToolsFromMCP(url: string, authHeaders?: Record<string, string>) {
  const result = await getToolsAndServerInfo(url, authHeaders)
  return result.tools
}
