...
async function inspectOpenAPI(url: string) {
  try {
    // Use api2 to inspect OpenAPI specification - uses env variable for local/production
    const api2Url = env.NEXT_PUBLIC_API2_URL
    const response = await fetch(`${api2Url}/inspect-mcp?url=${encodeURIComponent(url)}`)
