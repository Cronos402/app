...
import { CRONOS_NETWORK, USDC_ADDRESS } from "@/lib/constants/cronos"
...
function createMockRegistration(serverId: string): ServerRegistrationData {
  ...
  // USDC.e on Cronos testnet - using centralized constants
  const pricing: PricingEntry[] = [
    { assetAddress: USDC_ADDRESS.TESTNET, network: CRONOS_NETWORK.TESTNET, tokenDecimals: 6, maxAmountRequiredRaw: '100000', active: true },
  ]
  return {
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            <TokenDisplay
                              ...
                              network={(tool.pricing as PricingEntry[])?.[0]?.network || CRONOS_NETWORK.TESTNET}
                              amount={(() => {
                                const paymentInfo = tool.pricing as PricingEntry[] | undefined;
                                const rawAmount = paymentInfo?.[0]?.maxAmountRequiredRaw;
                                const network = paymentInfo?.[0]?.network || CRONOS_NETWORK.TESTNET;
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center justify-between text-xs">
                              ...
                              <Badge variant="outline" className="text-xs py-0 px-2">
                                {(tool.pricing as PricingEntry[] | undefined)?.[0]?.network || CRONOS_NETWORK.TESTNET}
                              </Badge>
                            </div>
                          </div>
                        </div>
