...
import env from "@/env";
import type { Price } from "cronos402";
import type { UnifiedNetwork } from "@/lib/commons/networks";
import { CRONOS_NETWORK } from "@/lib/constants/cronos";
...
export const urlUtils = {
  ...
  getApi2Url: () => env.NEXT_PUBLIC_API2_URL!,
  getFacilitatorUrl: () => env.NEXT_PUBLIC_FACILITATOR_URL ?? 'http://localhost:3004',
...
export async function serviceApiCall<T = unknown>(
  ...
  console.log('[serviceApiCall] Making request:', { baseUrl, endpoint, url, method: options.method || 'GET' })
  ...
  try {
    ...
    console.log('[serviceApiCall] Response status:', response.status, response.statusText)
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = await response.json()
        console.log('[serviceApiCall] Error response data:', errorData)
        errorMessage = errorData.error || errorData.message || errorMessage
        errorDetails = errorData.details || errorData
      } catch {
        // Failed to parse error JSON, use status text
      ...
      // Include status code in error message for frontend handling
      const statusAwareMessage = `${response.status}: ${errorMessage}`
      console.error('[serviceApiCall] Request failed:', statusAwareMessage, errorDetails)
      const error = new Error(statusAwareMessage) as ApiError
      error.status = response.status
      error.details = errorDetails
      throw error
    ...
    console.log('[serviceApiCall] Success response:', data)
    return data
  } catch (error) {
    clearTimeout(timeoutId)

    console.error('[serviceApiCall] Caught error:', error)
...
export const authApi = {
  ...
  linkWallet: async (data: {
    ...
    return serviceApiCall(urlUtils.getAuthUrl(), '/api/wallets/link', {
      method: 'POST',
...
export const api = {
  ...
  getUserWalletsWithBalances: async (includeTestnet = true) => {
    ...
    // Define networks to check for each wallet - Cronos only
    const networks = includeTestnet
      ? [CRONOS_NETWORK.CRONOS, CRONOS_NETWORK.TESTNET]
      : [CRONOS_NETWORK.CRONOS]
        ...
        for (const network of networks) {
          try {
            ...
            if (balanceData && balanceData.usdc) {
              const isTestnet = network === CRONOS_NETWORK.TESTNET || network.includes('testnet')
  ...
  getLatestPayments: async (
    ...
    const mkItem = (i: number) => {
      ...
      return {
        ...
        tool,
        amountFormatted: amt,
        currency: net === CRONOS_NETWORK.TESTNET ? 'TCRO' : 'CRO',
        network: net,
        user,
        timestamp: ts,
        txHash: hash,
